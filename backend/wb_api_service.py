...
class WBApiService:
    ...
    async def get_new_orders_since(self, token: str, last_check: datetime):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if "items" not in orders_data: return []
    ...
    async def get_my_stocks(self, token: str):
        """Получение списка товаров пользователя через API остатков"""
        if not token: return []
        # Берем остатки с сегодняшнего дня
        today = datetime.now().strftime("%Y-%m-%dT00:00:00")
        
        async with aiohttp.ClientSession() as session:
             data = await self._get_stocks(session, token, today)
             # _get_stocks возвращает агрегат, нам нужен raw list
             # Поэтому дублируем логику запроса для списка
    ...
    async def _get_orders(self, session, token: str, date_from: str):
        ...
        except Exception: return {"count": 0, "sum": 0, "items": []}
    ...
    async def _get_stocks(self, session, token: str, date_from: str):
        ...
        except Exception: return {"total_quantity": 0}
