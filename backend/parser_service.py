...
class SeleniumWBParser:
    ...
    def _init_driver(self):
        ...
        try:
            ...
            if os.path.exists(system_driver): service = EdgeService(executable_path=system_driver)
            else: service = EdgeService(EdgeChromiumDriverManager().install())
            driver = webdriver.Edge(service=service, options=edge_options)
        except Exception as e:
            logger.error(f"Ошибка Selenium: {e}")
            raise e
        # ВОЗВРАЩАЕМ 120 СЕКУНД
    ...
    def get_product_data(self, sku: int):
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                while time.time() - start_wait < 60: # ВОЗВРАЩАЕМ 60 СЕКУНД ОЖИДАНИЯ ЦЕН
                ...
                last_error = str(e)
                logger.error(f"Ошибка {attempt}: {last_error}")
                continue
            finally:
