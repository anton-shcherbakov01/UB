...
# --- НОВАЯ ЗАДАЧА: NOTIFICATIONS ("ДЗЫНЬ!") ---
        ...
        for user in users:
            try:
                ...
                if new_orders:
                    ...
                    for o in new_orders[:3]: # Показываем только первые 3, чтобы не спамить
...
def check_new_orders():
    ...
    Запускается каждые 15 минут (чтобы не убить лимиты WB API).
    """
    # Создаем event loop для запуска асинхронного кода
