...
def analyze_reviews_task(self, sku: int, limit: int = 50, user_id: int = None):
    ...
    if product_data.get("status") == "error": return {"status": "error", "error": product_data.get("message")}
    ...
    if reviews:
        ...
        except Exception as e: ai_result = {"flaws": ["Ошибка"], "strategy": [str(e)]}
    else: ai_result = {"flaws": ["Нет отзывов"], "strategy": ["-"]}
    
    return {"status": "success", "sku": sku, "image": product_data.get('image'), "rating": product_data.get('rating'), "reviews_count": product_data.get('reviews_count'), "ai_analysis": ai_result}
