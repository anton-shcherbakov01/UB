    ...
    for sku, data in sku_map.items():
        try:
            ...
            result.append({
                ...
                "supply": supply_data,
                "meta": meta # <--- ВОТ ЭТА СТРОЧКА ВЕРНЕТ ФОТО И НАЗВАНИЯ
            })
        except Exception as e:
...
async def generate_pnl_pdf(
    ...
    # (Access checks same as above)
    date_from_dt = now - timedelta(days=30) # Simplify for brevity, logic same as above
    date_to_dt = now
    if date_from:
