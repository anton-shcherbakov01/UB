...
import { LayoutGrid, Wand2, DollarSign, Bell, User } from 'lucide-react';
...
const TabNav = ({ active, setTab }) => (
  <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 px-2 py-3 flex justify-between items-end z-50 pb-8 safe-area-pb shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
    ...
    {/* Уведомления (Вместо ИИ) */}
    <button onClick={() => setTab('notifications')} className={`flex flex-col items-center gap-1 w-[20%] transition-colors ${active === 'notifications' ? 'text-indigo-600' : 'text-slate-400'}`}>
      <Bell size={22} strokeWidth={active === 'notifications' ? 2.5 : 2} />
      <span className="text-[9px] font-bold">Уведомл.</span>
    </button>
