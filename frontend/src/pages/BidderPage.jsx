...
import { 
    ...
    AlertCircle, RefreshCw, Settings, Save, X, Search, 
    Gavel, TrendingUp
} from 'lucide-react';
import { API_URL, getTgHeaders } from '../config';

const BidderPage = () => {
    // --- State для Управления Кампаниями ---
    ...
    // --- State для Чеккера Аукциона (Новое) ---
    ...
    // --- Логика Чеккера Аукциона ---
    const handleCheckAuction = async () => {
        if (!auctionQuery) return;
        setAuctionLoading(true);
        setAuctionData(null);
        try {
            const res = await fetch(`${API_URL}/api/bidder/auction?query=${encodeURIComponent(auctionQuery)}`, { headers: getTgHeaders() });
            if (res.ok) {
                setAuctionData(await res.json());
            } else {
                alert("Ошибка получения данных аукциона");
            }
        } catch (e) {
            console.error(e);
        } finally {
    ...
    return (
        <div className="p-4 space-y-6 pb-32 animate-in fade-in max-w-lg mx-auto">
             ...
             <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-6 rounded-[32px] text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
                <div className="relative z-10">
                    <h1 className="text-2xl font-black flex items-center gap-2">
                        <Zap className="text-yellow-300" fill="currentColor" /> Биддер Pro
                    </h1>
            ...
            {/* --- БЛОК 1: Проверка Аукциона (НОВЫЙ) --- */}
            <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-2 mb-3">
                    <Gavel size={18} className="text-indigo-600"/>
                    <h3 className="font-bold text-slate-700">Чекер ставок</h3>
                </div>
                <div className="flex gap-2">
                    <input type="text" value={auctionQuery} onChange={e => setAuctionQuery(e.target.value)}
                        placeholder="Поиск по фразе..." className="flex-1 p-3 bg-slate-50 rounded-xl outline-none text-sm"/>
                    <button onClick={handleCheckAuction} disabled={auctionLoading}
                        className="bg-indigo-600 text-white p-3 rounded-xl font-bold min-w-[50px] flex items-center justify-center">
                        {auctionLoading ? <Loader2 className="animate-spin" size={18}/> : <Search size={18}/>}
                    </button>
                </div>

                {auctionData && (
                    <div className="mt-4 space-y-2 animate-in slide-in-from-top-2">
                        <h4 className="text-xs font-bold text-slate-400 uppercase">Топ-5 Ставок</h4>
                        {auctionData.ads.slice(0, 5).map((ad, i) => (
                            <div key={i} className="flex justify-between items-center text-sm p-2 bg-slate-50 rounded-lg">
                                <div className="flex gap-2">
                                    <span className="font-bold w-4">{ad.position}.</span>
                                    <span className="truncate max-w-[150px] text-slate-600">{ad.brand}</span>
                                </div>
                                <div className="font-bold text-indigo-600">{ad.cpm} ₽</div>
                            </div>
                        ))}
                        {auctionData.ads.length === 0 && (
                            <div className="text-center text-slate-400 text-xs py-2">Рекламы не найдено</div>
                        )}
                    </div>
                )}
            </div>

            {/* БЛОК 2: Список Кампаний */}
            <div>
            ...
            {logs.length > 0 && (
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-bold text-sm text-slate-400 uppercase tracking-wider">История ставок</h3>
                    </div>
